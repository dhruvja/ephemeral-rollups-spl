...
pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> ProgramResult {
    ...
    let [payer, bubblegum_escrow_pda, leaf_owner, leaf_delegate, tree_config_pda, merkle_tree_pda, bubblegum_program_id, compression_program_id, noop_program_id, system_program_id] =
        accounts
    else {
    ...
    let asset = get_asset_id(&merkle_tree_pda.key, args.nonce);
